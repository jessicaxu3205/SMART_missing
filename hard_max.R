...
#hard_max_only(testdata)
#hard_max_only_test(testdata)

#testdata<- as.data.frame(test[[1]])


#hard_max_only_test <- function(data)
#{
#  data <- as.data.frame(data_m)
  
#  q2_f   <- lm(Y ~ o1 + a1 + o1_a1 + a2 + o2_a2 + a1_a2, data = data)
  
#  q2   <- lm(Y ~ o1 + a1 + o1:a1 + a2 + o2:a2 + a1:a2, data = data)
  
#  lm(Y ~ o1 + a1  + a2 + Intercept+a3, data = data)
